...
export function useQueries() {
  ...
  function countChanges(query: Query, currentDate?: string, compareDate?: string): number {
    ...
    if (!currentData || !compareData) return 0;

    // Helper function to safely count changes
    const safeCountChanges = (currentItems: KeywordItem[] = [], compareItems: KeywordItem[] = []): number => {
      if (!Array.isArray(currentItems) || !Array.isArray(compareItems)) return 0;

      const changes = currentItems.filter(item => {
        const compareItem = compareItems.find(ci => ci.key === item.key);
        return !compareItem || item.value !== compareItem.value;
      });

      const removals = compareItems.filter(item => 
        !currentItems.find(ci => ci.key === item.key)
      );
    ...
    let count = 0;
    count += safeCountChanges(currentData.keywords, compareData.keywords);
    count += safeCountChanges(currentData.keywordCounts, compareData.keywordCounts);
    count += safeCountChanges(currentData.tags, compareData.tags);
